...
def define_model(trial,max_len = args.max_len, vocab_size = len(args.alphabet)+1, n_transformblocks = args.n_transformblocks, num_heads = args.num_heads, drate = args.drate, embed_dim =args.embed_dim, dense_dim = args.dense_dim):

    embed_dim = trial.suggest_int('embed_dim', 64, 256)
    n_transformblocks = trial.suggest_int('n_transformblocks', 4, 12)
    num_heads = trial.suggest_int('num_heads', 4, 12)
    drate = trial.suggest_float('drate', 0.05, 0.3)
    dense_dim = trial.suggest_int('dense_dim', 8, 64)
...
def objective(trial):
    ...
    es = k.callbacks.EarlyStopping(monitor='val_loss', mode='min', verbose=0, patience=20)
    ...
    history = model.fit(
                        ...
                        verbose=0,
                        shuffle = False,
                        callbacks=[es, his2, lr, optuna.integration.KerasPruningCallback(trial, "val_loss", interval=30)])
    return min(mse_res)
#############################################################################################################################
...
study = optuna.create_study(sampler = optuna.samplers.TPESampler(seed = 4124), pruner =optuna.pruners.HyperbandPruner())
study.optimize(objective, n_trials=50, show_progress_bar=True)

with open("study.dat", "wb") as f:
    pickle.dump(study, f)


optuna.visualization.plot_parallel_coordinate(study).write_image('parallel_coordinate.pdf')
optuna.visualization.plot_contour(study).write_image('contour.pdf')
optuna.visualization.plot_slice(study).write_image('slice.pdf')
optuna.visualization.plot_param_importances(study).write_image('param_importances.pdf')
optuna.visualization.plot_optimization_history(study).write_image('optimization_history.pdf')
